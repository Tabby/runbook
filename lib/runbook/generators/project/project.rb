module Runbook
  module Generators
    class Project < Thor::Group
      ...
      class_option :test, type: :string, enum: %w[rspec minitest],
                          default: 'rspec', desc: %(Test-suite, "rspec" or "minitest")
      class_option :ci, type: :string, enum: %w[github travis gitlab circle],
      ...
      def remove_unneeded_files
        ...
        @gemspec_file_contents = File.readlines(gemspec_file) if File.exist?(gemspec_file)
        ...
        @gemfile_file_contents = File.readlines(gemfile) if File.exist?(gemfile)
        remove_file(gemfile)
      ...
      def shared_lib_dir
        ...
        @shared_lib_dir = if options.key?('shared-lib-dir')
                            options['shared-lib-dir']
                          else
                            ask(msg.join("\n"))
                          end
      end
      ...
      def create_gemfile
        ...
        if @gemspec_file_contents
          gems = @gemspec_file_contents.grep(/  spec.add_development_dependency/).map do |line|
        ...
        gems = @gemfile_file_contents.grep(/^gem /).join
      ...
      def update_bin_console
        ...
        new_require = %(require_relative "../lib/#{_name}")
