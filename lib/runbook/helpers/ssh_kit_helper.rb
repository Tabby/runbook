module Runbook
  module Helpers
    module SSHKitHelper
      ...
      def render_ssh_config_output(ssh_config)
        ''.tap do |output|
          if (servers = ssh_config[:servers]).any?
            ...
            server_str = "#{server_str[0..38]}...#{server_str[-38..]}" if server_str.size > 80
          ...
          if (strategy = ssh_config[:parallelization][:strategy])
            ...
            in_str << ", wait: #{wait}" if %i[sequence groups].include?(strategy)
            output << "#{in_str}\n"
          end
      ...
      def with_ssh_config(ssh_config, &)
        user = ssh_config[:user]
        group = ssh_config[:group]
        as_block = _as_block(user, group, &)
      ...
      def _coordinator_options(ssh_config_parallelization)
        ssh_config_parallelization.clone.tap do |options|
          options[:in] = options.delete(:strategy) if options[:strategy]
        end
      end

      def _as_block(user, group, &)
        if user || group
          -> { as({ user:, group: }) { instance_exec(&) } }
        else
          -> { instance_exec(&) }
        end
      end
      ...
      def _with_block(env, &)
        if env
          -> { with(env) { instance_exec(&) } }
        else
          -> { instance_exec(&) }
        end
      end

      def _within_block(path, &)
        if path
          -> { within(path) { instance_exec(&) } }
        else
          -> { instance_exec(&) }
        end
      end

      def _with_umask(umask, &)
