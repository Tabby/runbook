    ...
    module TmuxHelper
      ...
      def setup_layout(structure, runbook_title:)
        ...
        if File.exist?(layout_file)
          ...
          return stored_layout if _all_panes_exist?(stored_layout)
        end
      ...
      def kill_all_panes(layout_panes)
        ...
        layout_panes.each_value do |pane_id|
      ...
      def _layout_file(runbook_title)
        ...
        "#{Dir.tmpdir}/runbook_layout_\#{pid}_\#{session_name}_\#{pane_pid}_\#{pane_id}_#{runbook_title}.yml"`.strip
      end
      ...
      def _session_layout_files
        session_layout_glob =
          `tmux display-message -p "#{Dir.tmpdir}/runbook_layout_\#{pid}_\#{session_name}_*.yml"`.strip
      ...
      def _swap_runbook_pane(panes_to_init, layout_panes)
        return unless (runbook_pane = panes_to_init.find { |pane| pane[:runbook_pane] })
