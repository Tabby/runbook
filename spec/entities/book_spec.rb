...
RSpec.describe Runbook::Entities::Book do
  ...
  context 'with tags' do
    let(:tags) { [:suse] }
    let(:book) { Runbook::Entities::Book.new(title, tags:) }
  ...
  context 'with labels' do
    ...
    let(:book) { Runbook::Entities::Book.new(title, labels:) }
  ...
  describe '#parallelization' do
    ...
    it 'takes an optional limit in servers per group' do
      book.parallelization(strategy: :groups, limit:)
      expect(book.ssh_config[:parallelization]).to include(
        strategy: :groups,
        limit:
      )
    ...
    it 'takes an optional wait time between runs in seconds' do
      book.parallelization(strategy: :sequence, wait:)
      expect(book.ssh_config[:parallelization]).to include(
        strategy: :sequence,
        wait:
      )
