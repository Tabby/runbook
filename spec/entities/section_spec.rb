...
RSpec.describe Runbook::Entities::Section do
  ...
  context 'with tags' do
    let(:tags) { [:suse] }
    let(:section) { Runbook::Entities::Section.new(title, tags:) }
  ...
  context 'with labels' do
    ...
    let(:section) { Runbook::Entities::Section.new(title, labels:) }
  ...
  describe '#parallelization' do
    ...
    it 'takes an optional limit in servers per group' do
      section.parallelization(strategy: :groups, limit:)
      expect(section.ssh_config[:parallelization]).to include(
        strategy: :groups,
        limit:
      )
    ...
    it 'takes an optional wait time between runs in seconds' do
      section.parallelization(strategy: :sequence, wait:)
      expect(section.ssh_config[:parallelization]).to include(
        strategy: :sequence,
        wait:
      )
