...
RSpec.describe 'Runbook::Run' do
  ...
  describe 'runbook__statements__ask' do
    let(:prompt) { 'Sky blue?' }
    let(:into) { :sky_color }
    let(:object) { Runbook::Statements::Ask.new(prompt, into:) }
    ...
    context 'noop' do
      ...
      context 'when default specified' do
        ...
        let(:object) do
          Runbook::Statements::Ask.new(prompt, into:, default:)
        end
      ...
      context 'when echo: false specified' do
        let(:object) do
          Runbook::Statements::Ask.new(prompt, into:, echo: false)
        end
    ...
    context 'when default specified' do
      ...
      it 'passes the default value to the ask statement' do
        result = 'result'
        expect(toolbox).to receive(:ask).with(prompt, default:, echo: true).and_return(result)
  ...
  describe 'runbook__statements__ruby_command' do
    let(:block) { ->(_object, _metadata, _run) { raise 'This happened' } }
    ...
    context 'arguments' do
      let(:block) { ->(_object, _metadata, run) { raise unless run < Runbook::Run } }
  ...
  describe 'runbook__statements__tmux_command' do
    ...
    let(:metadata_override) { { layout_panes: } }
    let(:object) do
